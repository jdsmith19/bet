## TO DO

# SMALL

Be smarter about home / away splits -- take into account neutral site games (probably just use all away features instead?)
	* Need to add a neutral site flag on scrape ✅
	* Filter those out from the aggregates

Add debug mode that will run with much fewer games to make sure the code completes

Add TQDM, fewer print calls

Create a method for adding to matchup_details to avoid duplicating code

# MEDIUM

Add prediction markets
Injury Adjustment Agent needs some serious work
	* I don't trust the impact scores, need to dig into how they're created
	* Let the code generated by Claude handle the scraping, I'll handle the report
	* How do I deal with the fact that a player may have been injured all season or for the full window of features?
	* Refactor get_expert_analysis to handle multiple analysis an expert types
Save Predictions to DB and run analysis against results
Run the whole thing 10 times and run it through a consolidation agent
Integrate LangGraph
	* Maybe start with the loop to run the whole thing multiple times, then roll out to the existing orchestration agents?

# LARGE

Find better options for Expert Analysis

# VERY LARGE

Add travel data points

## DONE
Add Logistic Regression
Add Random Forest Regressor
Return data from all predictions in a JSON dictionary format
Create a parent class for all prediction engines that the individual classes inherit from
Add season reset to ELO rating.
Add rolling aggregates for at least [3,5,7] days
Add RPI
Add Home / Away Splits
Build an agent to test and refine model results across all available features
Build an agent to predict upcoming games based on multiple factors
Build an agent to extract expert analysis from external websites
Split tasks into subagents
Incorporate injury reports
Switch to OpenAI Library + DGXSpark
Change how I pull upcoming games
	* Pull all upcoming games from ProFootballReference
	* Include Week #
	* Use query of current data set of complete games to find upcoming games
Add Guess the Lines with Cousin Sal
	* Find
	* Download
	* Transcribe
	* Segment
	* Summarize by game