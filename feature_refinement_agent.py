...
class FeatureOptimizerAgent:
		...
		while self.experiment_count < self.max_experiments:
			...
			if response['message'].get('tool_calls'):
				...
				for tool_call in response['message']['tool_calls']:
					...
					# Print summary
					self.__print(result_summary(result))
			else:
	...
	def _get_system_prompt(self):
		...
		You have multiple window lengths (L3, L5, L7) for rolling statistics:
	...
	def __get_checkpoint_prompt(self):
		...
		best_summary = "\n".join([
			f"- {model}: {self._format_metric(result)}"
				...
				'parameters': {
					...
					'properties': {
						...
						'features': {
							'type': 'array',
							'items': {'type': 'string'},
							'description': 'List of feature names to include. Valid features: elo, rpi, days_rest, avg_points_scored_l3/l5/l7, avg_pass_adjusted_yards_per_attempt_l3/l5/l7, avg_rushing_yards_per_attempt_l3/l5/l7, avg_turnovers_l3/l5/l7, avg_penalty_yards_l3/l5/l7, avg_sack_yards_lost_l3/l5/l7, avg_points_allowed_l3/l5/l7, avg_pass_adjusted_yards_per_attempt_allowed_l3/l5/l7, avg_rushing_yards_per_attempt_allowed_l3/l5/l7, avg_turnovers_forced_l3/l5/l7, avg_sack_yards_gained_l3/l5/l7, avg_point_differential_l3/l5/l7'
						}
					},
	...
	def _agent_wants_to_stop(self, content):
	...
	def _print_final_summary(self):
		...
		for model_name in ['XGBoost', 'LinearRegression', 'RandomForest', 'LogisticRegression', 'KNearest']:
			if model_name in self.best_results:
				...
				print(f"{model_name}:")
				print(f"  {self._format_metric(result)}")
